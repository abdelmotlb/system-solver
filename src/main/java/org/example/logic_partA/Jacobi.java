...
import org.example.GUI.wayToSolve;
...
public class Jacobi {
    ...
    private double MaxRelativeError(double[] Old, double[] New) {
        double max = -1.0;
        for (int i = 0; i < New.length; i++) {
            double error = Math.abs(New[i] - Old[i]) / Math.abs(New[i]) * 100;
            if (error > max)
                max = error;
        }
    ...
    public double[] solve(double[][] cofficient, double[] B, double[] InitialGuess, double Noi_Re, boolean isNum) {
        ...
        try {
            if (isNum) {
                ...
                } while (MaxRelativeError(Old, New) > RelativeError && counter < wayToSolve.maxIterationsValue);
